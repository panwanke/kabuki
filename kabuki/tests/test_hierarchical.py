...
class TestModelCreation(unittest.TestCase):
    ...
    def setUpClass(self):
        ...
        data, _ = kabuki.generate.gen_rand_data(
            gen_func_df,
            {"A": {"loc": 0, "scale": 1}, "B": {"loc": 0, "scale": 1}},
            subjs=self.n_subj,
        )
    ...
    def test_simple_deps(self):
        ...
        n_nodes = 2 * (
            1 + self.n_subj * 2
        )  # n_conds * (mu_g + n_subj * (mu_subj + like))
        self.assertEqual(len(m.nodes_db), n_nodes)
    ...
    def test_assertion_on_wrong_param_name_tofix(self):
        ...
        HNodeSimple(
            self.data, depends_on={"non_existant": "condition", "mu": "condition"}
        )

    def test_simplevar_partly_deps(self):
        m = HNodeSimpleVar(self.data, depends_on={"mu": "condition"})
        n_nodes = (
            1 + 1 + 2 * (1 + self.n_subj * 2)
        )  # mu_std + mu_tau + n_conds * (mu_g + n_subj * (mu_subj + like))
        self.assertEqual(len(m.nodes_db), n_nodes)

    def test_simplevar_deps(self):
        m = HNodeSimpleVar(
            self.data, depends_on={"mu": "condition", "mu_std": "condition"}
        )
    ...
    def test_simplevar_double_deps_A(self):
        m = HNodeSimpleVar(
            self.data, depends_on={"mu": "condition", "mu_std": "condition2"}
        )
    ...
    def test_simplevar_double_deps_B(self):
        m = HNodeSimpleVar(
            self.data,
            depends_on={"mu": ["condition", "condition2"], "mu_std": "condition2"},
        )
...
class TestEstimation(unittest.TestCase):
    ...
    def test_map_approx(self):
        subjs = 40
        data, params_true = kabuki.generate.gen_rand_data(
            gen_func_df,
        ...
        for condition, subj_params in params_true.items():
            ...
            for idx, params in enumerate(subj_params):
                ...
                for param_name, value in params.items():
                    ...
                    assert (
                        len(node) == 1
                    ), "Only one node should have been left after slicing."
                    abs_error = np.abs(node.map[0] - value)
                    self.assertTrue(abs_error < 0.2)
                    counter += 1
...
class TestConcatenate(unittest.TestCase):
    def test_concat(self):
        n_subj = 5
        data, params = kabuki.generate.gen_rand_data(
            gen_func_df,
            {"A": {"loc": 0, "scale": 1}, "B": {"loc": 0, "scale": 1}},
            subjs=n_subj,
        )
